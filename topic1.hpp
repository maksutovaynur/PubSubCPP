...
public:
    ...
    static TopPtr spawn(const std::string &name, ui msg_count) {
        std::unique_ptr<Topic> t(new Topic(name, msg_count));
        if (t->start(false, false)) return t;
        else return nullptr;
    }

    static TopPtr spawn(const std::string &name) {
        std::unique_ptr<Topic> t(new Topic(name, 0));
        if (t->start(false, true)) return t;
        else return nullptr;
    }
    ...
    static TopPtr spawn_create(const std::string &name, ui msg_size, ui msg_count) {
        ...
        if (t->start(true, false)) return t;
        else return nullptr;
    }
...
//            if (tpc::interrupted) exit(0);
    ...
    static const ui MSG_SIZE = sizeof(T);

private:
    Topic(const std::string &name, ui msg_count) {
        tpc::init_system();
        this->name = name;
    ...
    bool start(bool create, bool ign_count) {
        ...
        if (existed) {
            ...
            if (MSG_SIZE != hdr->msg_size) {
                return tpc::Err("Proposed message size doesn't match existing topic message size");
            }
